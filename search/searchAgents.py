...
def foodHeuristic(state, problem):
    ...
    for food in foods:
        if not food in visitedFoods:
            unvisitedFoods.append(food)
            
    currentPosition = position
    """
    while len(unvisitedFoods) > 0:
        distance, food = min([(mazeDistance(currentPosition, food, gameState), food) for food in unvisitedFoods])
        sum += distance
    ...
    for food in foods:
        distance = mazeDistance(currentPosition, food, gameState)
        if distance > sum:
            sum = distance
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        distance, food = min([(util.manhattanDistance(currentPosition, food), food) for food in unvisitedFoods])
        isGoal = state == food
        return isGoal
        "*** YOUR CODE HERE ***"
        util.raiseNotDefined()
